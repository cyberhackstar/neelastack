...
export class AdminInquiryDetailComponent implements OnInit {
  ...
  inquiry = signal<Inquiry | null>(null); quotations = signal<Quotation[]>([]); creating = signal(false); sendingId = signal<string | null>(null); creatingEngagement = signal(false); engagementCreated = signal(false); engagementError = signal<string | null>(null); quotationError = signal<string | null>(null); copiedId = signal<string | null>(null);
  ...
  form = this.fb.nonNullable.group({
    title:['',Validators.required], scopeSummary:[''], currency:['INR',[Validators.required,Validators.maxLength(8)]], validUntil:[''], notes:[''], executiveSummary:[''], deliverables:[''], timeline:[''], paymentTerms:['30% project initiation\n40% core milestone\n30% production launch'], assumptions:['Client provides required content, credentials and timely feedback.'], exclusions:['Third-party subscription fees and material scope changes are excluded unless stated above.'], nextSteps:['Review the secure proposal\nAccept or request a discussion\nSchedule project kickoff'], lineItems:this.fb.array([this.buildLineItem()]),
  });
  get lineItems(): FormArray { return this.form.get('lineItems') as FormArray; }
  private buildLineItem(){return this.fb.nonNullable.group({description:['',Validators.required],amount:[0,[Validators.required,Validators.min(1)]]});}
  ...
  sendQuotation(id:string):void{this.sendingId.set(id);this.inquiryService.sendQuotation(id).subscribe({next:u=>{this.sendingId.set(null);this.quotations.set(this.quotations().map(q=>q.id===u.id?u:q));const i=this.inquiry();if(i)this.inquiry.set({...i,status:'QUOTED'});},error:()=>this.sendingId.set(null)});}
