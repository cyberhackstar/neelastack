...
export class BookingPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.bookingService.getMeetingType(slug).subscribe({
      ...
      error: () => {
        this.loadError.set(
          "We couldn't find that meeting type. It may no longer be open for booking.",
        );
        this.loading.set(false);
      },
    });
  ...
  loadAvailability(slug: string): void {
    ...
    this.bookingService
      .getAvailability(
        slug,
        fmt(from, this.timezone),
        fmt(to, this.timezone),
        this.timezone,
      )
      .subscribe({
        next: (days) => {
        ...
        error: (err) => {
          ...
          this.availabilityError.set(
            err?.error?.message ||
              "We could not load availability right now. Please try again.",
          );
        },
      });
  }
  ...
  googleCalendarUrl(booking: BookingDto): string {
    const toGoogleDate = (value: string): string =>
      new Date(value).toISOString().replace(/[-:]/g, "").replace(".000Z", "Z");
  ...
  submit(): void {
    ...
    const inquiryId =
      (this.detailsForm.get("inquiryId")?.value as string | undefined) ||
      undefined;
