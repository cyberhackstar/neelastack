...
/**
 * Exercises /api/v1/auth/** end to end: register, login, refresh, logout, and the
 * failure/edge cases the audit specifically called out (item #2 — "Authentication
 * integration tests"). Every request here goes through the real security filter chain,
...
class AuthIntegrationTest extends AbstractIntegrationTest {
    ...
    void refresh_withRevokedToken_returns400() throws Exception {
        ...
        when(tokenRevocationService.isRevoked(eq(jti))).thenReturn(true);
    ...
    void refresh_rotatesTheOldRefreshTokenSoItCannotBeReused() throws Exception {
        // A successful refresh must retire the token that was just spent — that's what makes
        ...
        org.mockito.Mockito.verify(tokenRevocationService).revoke(eq(jti), org.mockito.ArgumentMatchers.any());
    }

    @Test
    void refresh_reusingAnAlreadyRotatedToken_revokesTheWholeSessionFamily() throws Exception {
        // Simulates the theft scenario: a refresh token that was already rotated out (jti is
        // revoked) gets replayed. The response must be the same generic 400 as any other
        // revoked token (no information leak distinguishing "reuse" from "logged out"), but
        ...
        org.mockito.Mockito.verify(tokenRevocationService)
                .revokeFamily(eq(familyId), org.mockito.ArgumentMatchers.any());
    }

    @Test
    void refresh_whenSessionFamilyIsRevoked_isRejectedEvenWithAFreshJti() throws Exception {
        // A family-level revocation (triggered by the reuse-detection case above) must shut
        // down every token in that family, not just the specific jti that triggered it — this
        // is what forces the whole session to re-authenticate rather than limping along on
        // whichever token happened to be in the attacker's or the legitimate client's hands.
    ...
    void logout_returns204AndRevokesTheToken() throws Exception {
        ...
        // logout() only calls tokenRevocationService.revoke(...); since that's mocked out
        // (see AbstractIntegrationTest), verify the service actually asked for revocation
