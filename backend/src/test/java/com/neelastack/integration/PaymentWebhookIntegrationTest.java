...
class PaymentWebhookIntegrationTest extends AbstractIntegrationTest {
         ...
         * a PENDING invoice tied to orderId.
         */
        ...
        /**
         * Creates an admin user directly (mirrors AuthorizationIntegrationTest's
         * pattern) and returns
         * a bearer access token for it.
         *
         * The webhook-events replay route (POST /api/v1/admin/payments/**) is a
         * "high-risk mutation"
         * per StepUpAuthFilter, which requires both (a) MFA enrolled and (b) a *recent*
         * step-up
         * assertion — a valid JWT alone is not enough. We grant both directly here
         * rather than
        ...
        private String adminAccessToken(String email) throws Exception {
                // Created with MFA off so the plain login below actually returns a token pair
                // instead of an MFA challenge (AuthService#login short-circuits to
                // mfaRequired/mfaToken-only whenever isMfaEnabled() is true — see
                // AuthService.java).
                ...
                // Now flip MFA on so StepUpAuthFilter's user.isMfaEnabled() check passes.
                // StepUpAuthFilter re-fetches the user by email on every request, so this
                // update
                // takes effect immediately for the already-issued JWT above.
                ...
                // /api/v1/admin/mfa/verify + /step-up here would just be testing MfaController
                // again.
        ...
        void duplicateDelivery_ofAlreadyProcessedEvent_isNoOp() throws Exception {
                ...
                // Must not have been reprocessed a second time — attemptCount stays at 1, and
                // the row
        ...
        void adminReplay_ofFailedEvent_reprocessesAndMarksProcessed() throws Exception {
                // Simulates the operational scenario the review asked for (item #3): an event
                // that
                // previously FAILED (here, seeded directly rather than forced through a real
                // failure)
                ...
                String body = mockMvc
                                .perform(post("/api/v1/admin/payments/webhook-events/" + failedEvent.getId()
                                                + "/replay")
