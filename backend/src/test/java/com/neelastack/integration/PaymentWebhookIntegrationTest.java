...
/**
 * Exercises /api/v1/payments/webhook and the admin replay endpoint end to end —
 * the "webhook
 * replay" and "payment verification" HTTP-level coverage the review
 * specifically listed as
 * missing (item #5). Uses the real signature-verification code path (a genuine
 * HMAC-SHA256
...
class PaymentWebhookIntegrationTest extends AbstractIntegrationTest {
        ...
        /**
         * Creates a client user (via the real register endpoint) plus an engagement and
         * a PENDING
         * invoice tied to orderId.
         */
        private Invoice pendingInvoiceForOrder(String orderId) throws Exception {
                ...
                Engagement engagement = engagementRepository.save(
                                Engagement.builder()
                                                .client(client)
        ...
        /**
         * Creates an admin user directly and returns a valid bearer access token for
         * it.
         *
         * The webhook-events replay route (POST /api/v1/admin/payments/**) is a
         * "high-risk mutation"
         * protected by StepUpAuthFilter, which requires:
         *
         * 1. MFA to be enabled for the user, and
         * 2. a recent MFA step-up assertion.
         *
         * This test is specifically testing webhook replay authorization and processing
         * behavior,
         * not the HTTP MFA enrollment/login flow. Those flows are covered separately by
         * the
         * authentication/MFA tests, so we establish the required security state
         * directly here.
         *
         * We must NOT call /api/v1/auth/login here and expect an accessToken, because
         * an
         * MFA-enabled user receives an MFA challenge from the login endpoint first.
         * Instead,
         * generate the JWT directly for this integration test.
         */
        private String adminAccessToken(String email) {
                ...
                // Mirrors MfaService#grantStepUp's private key format.
                // This represents a recently completed MFA step-up for this test user.
                redisTemplate.opsForValue().set(
                                "mfa_step_up:" + admin.getId(),
                                "1",
        ...
        void validPaymentCaptured_marksInvoicePaidAndRecordsEventAsProcessed() throws Exception {
                ...
                assertThat(reloaded.getRazorpayPaymentId())
                                .isEqualTo("pay_ok_1");

                PaymentWebhookEvent event = webhookEventRepository.findByRazorpayEventId("evt_ok_1").orElseThrow();

                assertThat(event.getStatus())
                                .isEqualTo(PaymentWebhookEvent.WebhookEventStatus.PROCESSED);
        ...
        void duplicateDelivery_ofAlreadyProcessedEvent_isNoOp() throws Exception {
                ...
                /*
                 * Must not have been reprocessed a second time.
                 * attemptCount remains 1 because the second delivery is a duplicate
                 * of an already processed event.
                 */
        ...
        void adminReplay_ofFailedEvent_reprocessesAndMarksProcessed() throws Exception {
                /*
                 * Simulates the operational scenario the review asked for:
                 * an event that previously FAILED gets manually replayed by an admin
                 * and succeeds this time.
                 */
                Invoice invoice = pendingInvoiceForOrder("order_replay_1");

                PaymentWebhookEvent failedEvent = webhookEventRepository.save(
                                PaymentWebhookEvent.builder()
                                                .razorpayEventId("evt_replay_1")
                                                .eventType("payment.captured")
                                                .razorpayPaymentId("pay_replay_1")
                                                .razorpayOrderId("order_replay_1")
                                                .rawPayload(capturedPayload(
                                                                "order_replay_1",
                                                                "pay_replay_1"))
                ...
                String body = mockMvc.perform(
                                post("/api/v1/admin/payments/webhook-events/"
                                                + failedEvent.getId()
                                                + "/replay")
                                                .header(
                                                                "Authorization",
                ...
                assertThat(json.get("attemptCount").asInt())
                                .isEqualTo(2);
        ...
        void adminReplay_ofAlreadyProcessedEvent_isRejected() throws Exception {
                PaymentWebhookEvent processedEvent = webhookEventRepository.save(
                                PaymentWebhookEvent.builder()
